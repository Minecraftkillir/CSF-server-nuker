...
async def help(ctx):
    ...
    embed.add_field(name="\u200b", value="\u200b", inline=True)
    embed.add_field(name="Edit server icon", value=f">>> Image is attachment\n`{p}12`", inline=True)
    ...
    # embed.add_field(name="\u200b", value="\u200b", inline=True)
    embed.add_field(name="Revive (DM Only)", value=f">>> Creating 1 text channel on server if you deleted all\n`{p}15 <guild id>`", inline=False)
...
async def settings(ctx):
    p = bot.command_prefix
    embed = discord.Embed(
        title="Settings", description="Available settings", color=0xFF0000)
